;(function ($, window, document, undefined) {
  ...
  Foundation.libs.abide = {
      ...
      for (var i = count - 1; i >= 0; i--) {
        ...
        if (is_radio && required) {
          ...
          if (el_patterns[i][1].test(value) && valid_length ||
            ...
            $(el).removeAttr('data-invalid').parents('.data-abide-input-container').removeClass('error');
            validations.push(true);
          } else {
            $(el).attr('data-invalid', '').parents('.data-abide-input-container').addClass('error');
    ...
    valid_radio : function (el, required) {
      ...
      for (var i=0; i < count; i++) {
        if (valid) {
          $(group[i]).removeAttr('data-invalid').parents('.data-abide-input-container').removeClass('error');
        } else {
          $(group[i]).attr('data-invalid', '').parents('.data-abide-input-container').addClass('error');
        }
      }
